...
import { singleUser } from "../../api-calls/user-api";
...
const Profile = () => {
  ...
  const { data: user, isPending } = useQuery({
    ...
    queryFn: async () => await singleUser(userId),
  });
  ...
  return (
    <section className="bg-profileBg bg-cover bg-no-repeat lg:h-[490px] relative">
      <div className="lg:fixed lg:left-0 lg:right-0 lg:top-[80px] lg:flex">
        ...
        <div className="w-full lg:flex-[3] 1114:flex-[3.5] lg:overflow-y-auto lg:h-screen relative">
          {isPending && (
            <p className="absolute top-[5%] left-[50%] bg-white shadow-sm px-4 py-2 rounded-md">
              Loading...
            </p>
          )}
          ...
          <div className=" bg-slate-100 mb-[5rem] ">
            <div className="flex flex-col lg:flex-row gap-5 mb-[5rem] p-6">
              ...
              <div className="flex-1">
                <Social user={user} />
                {currentUser._id === userId && <Password />}
              </div>
            </div>
